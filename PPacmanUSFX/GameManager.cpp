...
int GameManager::onExecute() {
	...
	for (int i = 0; i <= 4; i++) // fantasmasvec.
	{
		srand(time(NULL));
		int w = rand() % 2;
		int t = 1 + rand() % (6 - 1);
	...
	while (juego_en_ejecucion) {
		...
		// Funcion de movimiento de pacman
		pacman->move();

		// Funcion de movimiento del fant.
		for (int i = 0; i <= 4; i++)
		{
			fantasma[i]->move();
		}
...
SDL_Texture* GameManager::loadTexture(string path)
{
	...
	{
		// textura en una superficie de pixeles
